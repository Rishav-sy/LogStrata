...
import { SaveScenarioControl } from "@/components/dashboard/SaveScenarioControl";
import { createClient } from "@/lib/supabase/client";
import { isSupabaseConfigured } from "@/lib/supabase/env";
import { scenarioConfigurationSchema, type ScenarioConfiguration } from "@/lib/validation/scenario";
import { 
...
const INITIAL_CONTAINER_CREATED_AT = 0;
...
export default function Dashboard() {
  ...
  const [containers, setContainers] = useState<ContainerPod[]>([
    { id: "1", name: "pod-ingress-7fc1", status: "running", createdAt: INITIAL_CONTAINER_CREATED_AT },
    { id: "2", name: "pod-ingress-8bd2", status: "running", createdAt: INITIAL_CONTAINER_CREATED_AT },
    { id: "3", name: "pod-ingress-3ea9", status: "running", createdAt: INITIAL_CONTAINER_CREATED_AT },
  ]);
  ...
  const [logFilter, setLogFilter] = useState<"ALL" | "INFO" | "WARN" | "ERROR" | "CRITICAL">("ALL");
  ...
  useEffect(() => {
    ...
    const loadScenario = async () => {
      ...
      const { data } = await supabase.from("saved_scenarios").select("configuration").eq("id", scenarioId).single();
      const parsed = scenarioConfigurationSchema.safeParse(data?.configuration);
      if (!parsed.success) return;
      const configuration = parsed.data;
      setTrafficMode(configuration.trafficMode);
      setRequestRate(configuration.requestRate);
      setDbFailure(configuration.failures.database);
      setApiFailure(configuration.failures.api);
      setAuthFailure(configuration.failures.auth);
      setTimeoutFailure(configuration.failures.timeout);
      setServiceCrash(configuration.failures.serviceCrash);
      setSliderCpu(configuration.resources.cpu);
      setCpu(configuration.resources.cpu);
      setSliderMemory(configuration.resources.memory);
      setMemory(configuration.resources.memory);
      setSliderDisk(configuration.resources.disk);
      setDisk(configuration.resources.disk);
      setSliderNetwork(configuration.resources.network);
      setNetwork(configuration.resources.network);
      setAutoScalingEnabled(configuration.autoScalingEnabled);
      setActiveAnomaly(configuration.activeAnomaly);
    };
    void loadScenario();
  }, []);
  ...
  const triggerAnomaly = (type: typeof activeAnomaly) => {
    ...
    if (type === "spike") {
      ...
      addTerminalLog("WARN", "traffic-monitor", "Anomaly alert: Traffic rate spiked 50x in < 2 seconds");
    } else if (type === "leak") {
      setLeakRate(1.5);
      triggerAlert("WARNING", "Memory Leak active on pod-ingress-* group");
      addTimelineEvent("ANOMALY: Memory Leak Triggered", "ALERT");
      addTerminalLog("WARN", "kube-oom-monitor", "Memory leak pattern flagged: steady heap rise of 1.5% per second");
    } else if (type === "storm") {
      setApiFailure(true);
      setAuthFailure(true);
  ...
  const handleRemoveContainer = () => {
    ...
    addTerminalLog("WARN", "orchestration-engine", `Kubernetes: Terminated pod replica ${target.name}`);
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
        ...
        setScenarioTimer((prev) => {
          ...
          if (activeScenario === "black_friday") {
            if (next === 5) {
              ...
              addTerminalLog("WARN", "traffic-monitor", "Flash sale load warning: scale threshold breached");
            } else if (next === 45) {
      ...
      if (autoScalingEnabled && !currentCrash && containers.length > 0) {
        ...
        if (finalCpu > scaleUpThreshold || requestRate > 4000) {
          if (containers.length < 12) {
            ...
            addTerminalLog("WARN", "autoscaler", `Scaling event triggered: CPU usage (${finalCpu}%) is above threshold (${scaleUpThreshold}%). Scaling up replica set.`);
          }
        }
      ...
      // 6. Append logs periodically
      if (Math.random() > 0.3) {
        if (currentDb) {
          addTerminalLog("CRITICAL", "db-connector", "Connection timeout on db-primary-0. Pool exhausted.");
          addTerminalLog("ERROR", "db-connector", "Connection pool fallback failed. Retrying in 500ms.");
        } else if (currentCrash || containers.length === 0) {
          addTerminalLog("CRITICAL", "orchestration-engine", "Service unavailable: 0 backend replicas responding");
        } else {
      ...
      // 7. Update Telemetry History Chart Data
      ...
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex flex-col items-end gap-2">
            <SaveScenarioControl configuration={{
              version: 1,
              trafficMode,
              requestRate,
              failures: { database: dbFailure, api: apiFailure, auth: authFailure, timeout: timeoutFailure, serviceCrash },
              resources: { cpu: sliderCpu, memory: sliderMemory, disk: sliderDisk, network: sliderNetwork },
              autoScalingEnabled,
              activeAnomaly,
            } satisfies ScenarioConfiguration} />
            {/* Alert Ticker */}
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className={`stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer ${
                    trafficMode === "normal" ? "border-primary bg-primary/5 text-primary" : ""
                  }`}
                >
                ...
                <button
                  ...
                  className={`stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer ${
                    trafficMode === "high" ? "border-warning bg-warning/5 text-warning" : ""
                  }`}
                >
                  <span>High Load</span>
                ...
                <button
                  ...
                  className={`stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer ${
                    trafficMode === "viral" ? "border-primary bg-primary/5 text-primary" : ""
                  }`}
                >
                  <span>Viral Post Surge</span>
                ...
                <button
                  ...
                  className={`stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer ${
                    trafficMode === "ddos" ? "border-error bg-error/5 text-error font-bold" : ""
                  }`}
                >
                  <span>DDoS Attack Flood</span>
            ...
            <div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex justify-between text-xs font-mono">
                    <span className="flex items-center gap-1.5 text-ink">
                      <Server className="h-3.5 w-3.5 text-mute" /> Disk Storage
                    </span>
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex justify-between text-xs font-mono">
                    <span className="flex items-center gap-1.5 text-ink">
                      <Wifi className="h-3.5 w-3.5 text-mute" /> Network Bandwidth
                    </span>
            ...
            <div>
              <div className="flex items-center justify-between border-b border-hairline pb-3 mb-4">
                ...
                <div className="flex items-center gap-1.5 text-xs font-semibold">
                  <span className="text-[10px] text-mute uppercase">Auto:</span>
                  <button
                    onClick={() => {
                      setAutoScalingEnabled(!autoScalingEnabled);
                      addTimelineEvent(`Autoscaling sandbox changed: ${!autoScalingEnabled ? "ENABLED" : "DISABLED"}`, "INFO");
                      addTerminalLog("INFO", "autoscaler", `Autoscaling rules engine status: ${!autoScalingEnabled ? "ONLINE" : "OFFLINE"}`);
                    }}
                    className={`relative inline-flex h-4 w-8 shrink-0 cursor-pointer rounded-[10px] border border-hairline transition-colors duration-200 ${
                      autoScalingEnabled ? "bg-primary border-primary" : "bg-canvas"
                    }`}
                  >
                    <span className={`pointer-events-none inline-block h-3 w-3 transform rounded-[8px] transition duration-200 bg-canvas ${
                      autoScalingEnabled ? "translate-x-3.5" : "translate-x-0 bg-mute"
                    }`} />
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 mb-4 text-center text-xs font-mono">
                <div className="bg-canvas-soft py-1.5 border border-hairline rounded-[6px]">
                  ...
                  <p className="text-[8px] text-mute uppercase tracking-wider mt-0.5">Active Pods</p>
                </div>
                <div className="bg-canvas-soft py-1.5 border border-hairline rounded-[6px] text-error">
                  ...
                  <p className="text-[8px] text-mute uppercase tracking-wider mt-0.5">Scale Up</p>
                </div>
                <div className="bg-canvas-soft py-1.5 border border-hairline rounded-[6px] text-link">
                  ...
                  <p className="text-[8px] text-mute uppercase tracking-wider mt-0.5">Scale Down</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-2 mb-4">
                {containers.map((c) => (
                  <div 
                    key={c.id} 
                    className={`p-1.5 border rounded-[4px] flex flex-col items-center justify-center text-center transition-all ${
                      c.status === "starting" 
                  ...
                  >
                    ...
                    <span className={`font-mono text-[6px] uppercase tracking-wide px-1 rounded-[2px] mt-0.5 ${
                      c.status === "starting" ? "bg-warning/10" : "bg-success/10"
                    }`}>
                      {c.status}
                    </span>
                  </div>
              ...
              <div className="flex gap-2">
                ...
                >
                  <Plus className="h-3.5 w-3.5 mr-1" /> Add Pod
                </button>
                <button
                  id="btn-scale-down"
                  ...
                  className="flex-1 stark-btn-secondary h-8 px-2.5 text-xs font-semibold cursor-pointer disabled:opacity-50"
                >
                  <Minus className="h-3.5 w-3.5 mr-1" /> Scale Down
                </button>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-2.5">
                ...
                <div className="flex items-center justify-between bg-canvas-soft p-2.5 border border-hairline rounded-[6px]">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-ink">Database Outage</span>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-4 w-8 shrink-0 cursor-pointer rounded-[10px] border border-hairline transition-colors duration-200 ${
                      dbFailure ? "bg-error border-error" : "bg-canvas"
                    }`}
                  >
                    <span className={`pointer-events-none inline-block h-3 w-3 transform rounded-[8px] transition duration-200 bg-canvas ${
                      dbFailure ? "translate-x-3.5" : "translate-x-0 bg-mute"
                    }`} />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between bg-canvas-soft p-2.5 border border-hairline rounded-[6px]">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-ink">API Gateway Failure</span>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-4 w-8 shrink-0 cursor-pointer rounded-[10px] border border-hairline transition-colors duration-200 ${
                      apiFailure ? "bg-error border-error" : "bg-canvas"
                    }`}
                  >
                    <span className={`pointer-events-none inline-block h-3 w-3 transform rounded-[8px] transition duration-200 bg-canvas ${
                      apiFailure ? "translate-x-3.5" : "translate-x-0 bg-mute"
                    }`} />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between bg-canvas-soft p-2.5 border border-hairline rounded-[6px]">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-ink">Auth Token Expiry</span>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-4 w-8 shrink-0 cursor-pointer rounded-[10px] border border-hairline transition-colors duration-200 ${
                      authFailure ? "bg-error border-error" : "bg-canvas"
                    }`}
                  >
                    <span className={`pointer-events-none inline-block h-3 w-3 transform rounded-[8px] transition duration-200 bg-canvas ${
                      authFailure ? "translate-x-3.5" : "translate-x-0 bg-mute"
                    }`} />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between bg-canvas-soft p-2.5 border border-hairline rounded-[6px]">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-ink">Upstream Timeout</span>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-4 w-8 shrink-0 cursor-pointer rounded-[10px] border border-hairline transition-colors duration-200 ${
                      timeoutFailure ? "bg-error border-error" : "bg-canvas"
                    }`}
                  >
                    <span className={`pointer-events-none inline-block h-3 w-3 transform rounded-[8px] transition duration-200 bg-canvas ${
                      timeoutFailure ? "translate-x-3.5" : "translate-x-0 bg-mute"
                    }`} />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between bg-canvas-soft p-2.5 border border-hairline rounded-[6px]">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-ink">Service Pod Crash</span>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-4 w-8 shrink-0 cursor-pointer rounded-[10px] border border-hairline transition-colors duration-200 ${
                      serviceCrash ? "bg-error border-error" : "bg-canvas"
                    }`}
                  >
                    <span className={`pointer-events-none inline-block h-3 w-3 transform rounded-[8px] transition duration-200 bg-canvas ${
                      serviceCrash ? "translate-x-3.5" : "translate-x-0 bg-mute"
                    }`} />
                  </button>
                </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                <button
                  ...
                  className={`stark-btn-secondary h-8 text-xs font-semibold cursor-pointer ${
                    activeAnomaly === "spike" ? "bg-warning/15 border-warning text-warning" : ""
                  }`}
                >
                  Traffic Spike
                </button>
                <button
                  ...
                  className={`stark-btn-secondary h-8 text-xs font-semibold cursor-pointer ${
                    activeAnomaly === "leak" ? "bg-warning/15 border-warning text-warning" : ""
                  }`}
                >
                  Memory Leak
                </button>
                <button
                  ...
                  className={`stark-btn-secondary h-8 text-xs font-semibold cursor-pointer ${
                    activeAnomaly === "storm" ? "bg-error/15 border-error text-error" : ""
                  }`}
                >
                  Error Storm
                </button>
                <button
                  ...
                  className={`stark-btn-secondary h-8 text-xs font-semibold cursor-pointer ${
                    activeAnomaly === "outage" ? "bg-error/15 border-error text-error font-bold" : ""
                  }`}
                >
                  Service Outage
                </button>
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className="stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer"
                >
                  <span>Black Friday Traffic Rush</span>
                ...
                <button
                  ...
                  className="stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer"
                >
                  <span>Database Crash Failover</span>
                  <Play className="h-3.5 w-3.5 text-mute" />
                </button>
                <button
                  id="demo-viral-post"
                  ...
                  className="stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer"
                >
                  <span>Viral Post Surge Campaign</span>
                  <Play className="h-3.5 w-3.5 text-mute" />
                </button>
                <button
                  id="demo-ddos-attack"
                  ...
                  className="stark-btn-secondary h-9 px-3 text-xs font-semibold justify-between cursor-pointer"
                >
                  <span>Distributed DDoS Flood</span>
                  <Play className="h-3.5 w-3.5 text-mute" />
                </button>
              </div>
            </div>
        ...
        {/* Live Cluster Metrics Chart Section */}
        <div className="stark-card mb-6">
          ...
          <div className="w-full h-[180px] bg-canvas-soft rounded-[6px] relative overflow-hidden border border-hairline">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between border-b border-hairline pb-4 mb-4 gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex rounded-[6px] border border-hairline bg-canvas p-0.5 overflow-hidden text-[10px] font-mono">
                {(["ALL", "INFO", "WARN", "ERROR", "CRITICAL"] as const).map((filter) => (
                  <button
                    key={filter}
                    onClick={() => setLogFilter(filter)}
                    className={`px-2 py-1 rounded-[4px] cursor-pointer font-bold transition-all ${
                      logFilter === filter ? "bg-primary text-on-primary" : "text-mute hover:text-ink"
                    }`}
                  >
                    {filter}
                  </button>
            ...
            {terminalLogs.length === 0 ? (
              ...
              terminalLogs
                ...
                .map((log) => {
                  ...
                  if (log.level === "WARN") badgeColor = "bg-warning/10 border-warning/20 text-warning";
